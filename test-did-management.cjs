...
async function testDIDManagement() {
  console.log('🚀 Testing DID Management page...');
  ...
  try {
    ...
    const phoneNumbers = await page.locator('table tbody tr td:first-child').allTextContent();
    console.log(`📱 Phone numbers found: ${phoneNumbers.length > 0 ? phoneNumbers.slice(0, 5).join(', ') : 'None'}`);
    ...
    // Take screenshot
    await page.screenshot({ path: 'test-did-management.png' });
    console.log('✅ Screenshot saved: test-did-management.png');
    ...
    await browser.close();
    console.log('✅ DID Management test completed');

  } catch (error) {
    console.error('❌ Test failed:', error.message);
    await page.screenshot({ path: 'test-did-management-error.png' });
    console.log('✅ Error screenshot saved: test-did-management-error.png');
    await browser.close();
  }
}
